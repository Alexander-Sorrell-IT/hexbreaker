...
| 2 | **≤5 min demo video** | The non-compliant PIL-slideshow MP4 was **removed** (commit `0059dc9`). An **uncommitted local candidate** `docs/demo.mp4` exists — a real **live-terminal screencast** (asciinema capture of the actual tool: the self-correction CONFIRMED→CONTESTED beat + the NIST 4/4 run) with neural-TTS narration, built by [scripts/build_screencast_demo.py](scripts/build_screencast_demo.py); recording plan in [docs/demo_runbook.md](docs/demo_runbook.md). | ⏳ **Pending the human gate.** Live terminal execution is genuine; narration is synthetic TTS (rules require "audio narration", not a specific voice). Decision needed: accept this take OR record a voiced take per the runbook → then commit + upload public + put the URL on Devpost. |
...
|---|---|---|
| Demo video not recorded by 6/13 | partially mitigated | A built live-terminal screencast candidate exists locally (`docs/demo.mp4`, regenerable via `scripts/build_screencast_demo.py`); needs Alex's accept-or-rerecord decision + public upload. |
