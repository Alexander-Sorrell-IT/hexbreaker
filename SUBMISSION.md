...
| 8 | **Structured agent execution logs** | every Court run produces hash-chained JSONL at `<case_dir>/transcript.jsonl` + per-tool sidecar files at `<case_dir>/transcript.outputs/`. HMAC signature optional via `hexbreaker sign`. Validators: `hexbreaker verify --transcript X.jsonl --hmac` (chain + HMAC integrity) and `hexbreaker trace --findings F.json --transcript X.jsonl` (traces **each finding** to the exact tool execution that produced it — joins cited steps → tool_call → re-hashed sidecar bytes, exits non-zero on a fabricated citation or tampered sidecar). **Committed sample:** `samples/self_correction/transcript.jsonl` (+ `.sig` + sidecars) — chain-verifiable via `hexbreaker verify --transcript samples/self_correction/transcript.jsonl` (HMAC too with `HEXBREAKER_HMAC_PASSWORD=demo-self-correction`); regenerate via `python scripts/demo_self_correction.py`. The committed NIST run traces 4/4 findings to `fls`/`icat` output: `hexbreaker trace --findings samples/nist_fsm_run/run1/findings.json --transcript samples/nist_fsm_run/run1/transcript.jsonl`. | ✅ ready |
